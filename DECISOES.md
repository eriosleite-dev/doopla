...
**Atualização 14/09/2026 (reconciliação de branch)**: a branch onde
essa decisão foi tomada estava desatualizada — não tinha o trabalho
registrado abaixo (Settings V2, ROADMAP MESTRE, Categoria A, etc.),
feito em paralelo numa branch diferente (`claude/new-session-3hdkui`)
e agora trazido pra cá via merge. A separação Production/QA-Staging
descrita acima continua válida e já foi executada de verdade (ver
`PROGRESS.md`, seção Categoria B) — inclusive a correção das
variáveis compartilhadas na Vercel.
