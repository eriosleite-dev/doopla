...
--
-- Nota de numeração: este trabalho foi desenvolvido numa sessão isolada
-- (branch claude/busy-wright-5z9kr8) que divergiu da canônica antes da
-- migration 0045 real desta branch (0045_approval_engine.sql). O SQL
-- abaixo é idêntico ao que já rodou nos ambientes reais (doopla-qa-
-- staging e doopla) sob o nome temporário 0045_contact_messages.sql —
-- só o número do arquivo muda aqui, pra não colidir com o histórico
-- desta branch. Não é preciso rodar este arquivo de novo nos ambientes
-- onde 0045_contact_messages.sql já foi aplicado (mesmo conteúdo, só
-- criaria erro de "já existe").
--
