...
- ✅ **PASS — Estrutura do acordeão em `/dashboard/perfil`**, confirmado
  ...
  **auditoria de UI pré-lançamento** (não especificados agora, não
  ...
  quando ela detalhar.
- ✅ **PASS — `/dashboard/perfil/dados`**, confirmado pela fundadora
  (14/09/2026): Subcategoria/Mercados de fato sumiram, os campos
  restantes salvam e persistem depois de recarregar.
  **Achado corrigido na hora**: nenhuma confirmação visual aparecia ao
  clicar "Salvar" — mesmo `FAIL NON-BLOCKER` já registrado antes da
  reestruturação, que fazia parte do escopo aprovado
  ("feedback visual consistente de salvamento") mas não tinha sido
  implementado em `pro-artist-identity-form.tsx`/
  `pro-work-context-form.tsx`. Corrigido: `updateArtistProfileAction`/
  `updateArtistWorkContextAction` agora retornam `{ success: true }`
...
- ✅ **RESOLVIDO em 14/09/2026** — `NEXT_PUBLIC_SUPABASE_URL`/
  ...
  único valor pros dois ambientes), contradizendo a arquitetura fixada
  em `DECISOES.md`. Como o valor antigo estava marcado "Secret"
  (write-only, não revelável), não dava pra saber qual dos dois
  projetos era — corrigido apagando as entradas antigas e recriando
  do zero como **Config** (tipo legível, evita esse problema de novo
  no futuro), com valores pegos direto na fonte (Project Settings →
  API de cada projeto no Supabase, não "preservando" o valor antigo
  desconhecido):
  - `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`,
    escopo **Production** → valores do projeto `doopla`
    (`ysjzhirlhuujxoysbava`).
  - `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`,
    escopo **Preview** → valores do projeto `doopla-qa-staging`
    (`ahsyoxjzxkxcsbhqmdzv`).
  Conferido revelando (Config permite) os 4 valores finais — refs
  batem com o esperado em cada ambiente. `SUPABASE_SERVICE_ROLE_KEY`
  já estava corretamente separada antes, não precisou mexer.
  **Efeito prático**: como variáveis `NEXT_PUBLIC_` do Next.js são
  gravadas no bundle só no momento do build, o site de Production já
  publicado não mudou nada com isso (sem risco durante a correção);
  o próximo deploy de Preview desta branch (e todos os futuros) já
  sai com os valores certos do `doopla-qa-staging`. **Isso também
  destrava o P0 de teste**: dá pra usar o link de Preview no
  navegador com segurança agora, sem depender de `npm run dev` local
  (resolve de quebra o bloqueio do Mac com macOS antigo abaixo).
...
**Pendência de roadmap registrada pela fundadora (15/09/2026)**: `what_you_do`/`where_you_serve`/`fee_range`/`issues_invoice` são os 4 critérios corretos pra tela "Como você trabalha" que existe HOJE — mas já existe uma revisão futura planejada de "Perfil e trabalho" (incluindo revisão de cachê/condições e auditoria dos campos canônicos). **Quando esse bloco futuro for executado, `getArtistMatchingCompletion` precisa ser revalidado de novo contra os campos finais da nova experiência** — os 4 critérios atuais não podem virar uma nova regra legada fixa só porque resolveram o bug de hoje. Registrado aqui pra não se perder: o mesmo tipo de bug (critério apontando pra coluna que a UI já não edita mais) só existe porque uma correção anterior nunca foi revisitada quando a tela mudou.

## Item 1 — Link de booking → Conversation — validação completa — `[DELIVERED + VALIDATED]` — 15/09/2026

Fechamento da validação pedida pela fundadora antes de seguir pro redesign de `/orcamento/[slug]`. Auditoria/implementação já registradas acima (migration `0082_public_link_conversation.sql`); esta seção documenta a validação funcional/segurança, com evidência real, não só leitura de código.

**Método de validação**: sem acesso de rede a `doopla-qa-staging` neste ambiente (constraint de toda a sessão), a migration chain completa (0001–0082) foi aplicada contra um Postgres local, com um bootstrap mínimo de stubs Supabase (`auth.uid()`/`auth.jwt()`/`auth.role()` lidos do mesmo GUC real `request.jwt.claims`, roles `anon`/`authenticated`/`service_role`, `storage.buckets`/`storage.objects`/funções de helper). Correção de metodologia no meio do processo: a primeira tentativa deixou `anon`/`authenticated`/`service_role` sem NENHUM grant de baseline, o que tornava "permission denied" trivialmente verdadeiro mesmo sem o `revoke` da migration fazer nada — corrigido replicando `alter default privileges ... grant all on tables/routines/sequences to anon, authenticated, service_role`, o mesmo comportamento de auto-grant que o Supabase real aplica por padrão (confirmado contra `grant_baseline_privileges.sql`, script já usado numa auditoria anterior desta mesma sessão pra corrigir `doopla-qa-staging`). Só com essa baseline permissiva o `revoke` explícito da migration 0082 vira um teste que prova algo de verdade.

**Resultado de cada check**:

1. **Fluxo WhatsApp/service_role idêntico** — ✅ confirmado. `create_conversation()` chamado como `service_role` (simulando o webhook), com `origin/channel='whatsapp'` e um `external_participant_id` real, cria a conversation normalmente.
2. **Extração pro núcleo não alterou o comportamento do WhatsApp** — ✅ confirmado, mesmo teste do item 1: `create_conversation()` mantém assinatura, checks e resultado byte-a-byte iguais aos de antes da 0082 (só passou a delegar o insert final pro núcleo).
3. **Segurança/grants/RLS do núcleo** — ✅ `_create_conversation_core` só tem `EXECUTE` pra `postgres` (dono); nenhum grant pra `anon`/`authenticated`/`service_role`, mesmo com a baseline permissiva de auto-grant ativa (a `revoke` explícita da migration realmente bloqueia). RLS de `conversations` continua isolando por `represented_professional_id`: um `authenticated` diferente do profissional dono não vê a conversation do pedido (0 linhas); o profissional dono vê (1 linha).
4. **`anon`/`authenticated` não conseguem chamar o núcleo direto** — ✅ confirmado nos dois papéis: `ERROR: permission denied for function _create_conversation_core`.
5. **`submit_orcamento_request` cria opportunity + conversation + vínculo + origin/channel corretos, mesma transação** — ✅ confirmado como `anon`: `opportunities` (status=aberta, source=artist_link) e `conversations` (origin=`public_link`, channel=`public_link`, `related_opportunity_id` apontando pra oportunidade certa, `origin_reference`=slug) nascem juntos; os 2 eventos de nascimento (`conversation_mandate_events`/`conversation_state_events`) gravados com `changed_by='system'`.
6. **Sem duplicação de conversation pra mesma opportunity** — ✅ confirmado: `select related_opportunity_id, count(*) ... having count(*) > 1` retorna 0 linhas; 1 opportunity criada, 1 conversation vinculada. Idempotência do ramo WhatsApp também confirmada à parte: 2 chamadas seguidas de `create_conversation` pro mesmo par (profissional, participante) devolvem o MESMO id de conversation (reuso via advisory lock, comportamento pré-existente, inalterado).
7. **Estado real de "Precisa de você" após a criação** — ✅ confirmado via `get_conversation_operational_facts()`: conversation nova tem `status='open'`, `has_pending_runtime_reply=false`, `last_outbound_intent_delivery_state=null`, `last_message_direction=null` — aplicando `deriveConversationState` isso cai em `in_progress` (nunca `needs_you`). Ou seja, um pedido recém-chegado pelo link **não** aparece como "Precisa de você" — ele só vira isso quando a Doopla efetivamente chegar a um ponto de decisão real. Confirma que a correção de "Precisa de você" (rodada anterior) funciona certo com uma conversation de verdade, não só com o caso "sem conversation" testado antes.
8. **Home, Bookings, detalhe e sidebar na mesma fonte canônica** — ✅ confirmado por auditoria de código: `resolveDooplaIntervention` (`doopla-intervention.ts`) só tem 3 chamadores (`work-items.ts` → lista de Bookings, `professional-home-view.tsx` → Home, `oportunidades/[id]/page.tsx` → detalhe) e `getCachedConversationOperationalFacts` (o wrapper `cache()` sobre a mesma RPC) é a fonte usada por Home, `layout.tsx` (badge do sidebar) e lista de Bookings — nenhuma reimplementação divergente.
9. **Typecheck/lint + limitações da validação SQL** — `npx tsc --noEmit` e `npx eslint .` limpos (nenhuma mudança de código nesta rodada de validação, só a migration já commitada antes). **Limitação registrada explicitamente**: esta validação prova a lógica SQL/PL-pgSQL e o modelo de privilégio contra uma réplica fiel do schema real (mesma migration chain, mesmo GUC que `auth.uid()`/`is_system_caller()` leem em produção) — **não** é um teste contra o Supabase real (`doopla-qa-staging`), que este ambiente não alcança por rede. Continua pendente uma confirmação de ponta a ponta pela fundadora no ambiente real (formulário `/orcamento/[slug]` de verdade → Supabase real) antes do fechamento de beta, mas nenhuma bandeira vermelha foi encontrada nesta validação.
...
| 2a | `requires_professional_review` (P0 de beta — infra fail-closed): `send-outbound-intents` auto-enviava qualquer `outbound_intent` sem distinguir mensagens que o Planner marcou `requiresProfessionalReviewBeforeSend=true` (ex.: `answer_with_known_information`, risco de dado de terceiro) de drafts trivialmente seguros. Corrigido e validado (migração 0083 + propagação em `pipeline.ts`/`resumption.ts`/`outbound.ts`/`pending-replies.ts`, 10/10 testes PASS, ver §107). Arquitetura aprovada pela fundadora: AUTO-SEND (`Gate allowed` + `requires_professional_review=false`) claimável; REVIEW REQUIRED (`Gate allowed` + `true`) retido, cron nunca claima; PROTECTED DECISION continua bloqueado pelo Approval Engine/Gate antes do outbound, intocado. | `DELIVERED + VALIDATED` | Sessão Central, P0 backend/E2E (§106/§107) | **CLOSED (infra)** |
| 2b | Mecanismo de liberação da mensagem retida (`requires_professional_review=true`) — ainda não existe nenhuma ação humana que libere o envio. Decisão explícita: nunca um sistema paralelo de aprovação, nunca workaround virando `false`, nunca RPC/tela nova sem definição de produto — liga ao Approval Engine existente, desenho concreto fica pra depois de a Sessão Painel decidir qual ação humana existe no produto. | `PENDING INTEGRATION — aguarda decisão de produto (Sessão Painel)` | Sessão Central, §107 | **OPEN** |
| 2c | `needs_you` (`state.ts`/`doopla-intervention.ts`) ainda não distingue `requires_professional_review` — continua disparando pra qualquer `lastOutboundIntentDeliveryState==='policy_allowed'`, retido ou não. Só o comentário obsoleto foi corrigido nesta rodada (texto, zero mudança de comportamento). Não alterado de propósito: `doopla-intervention.ts` é compartilhado, alimenta Professional Home, e a arquitetura desta superfície está sendo auditada em paralelo pela Sessão Painel. | `PENDING INTEGRATION — aguarda auditoria da Sessão Painel` | Sessão Central, §107 | **OPEN** |
| 3 | `_create_conversation_core` (migração 0082): o `revoke` cobria só `anon, authenticated`, nunca `service_role`. Corrigido e validado (migração 0084, §107): `revoke execute ... from service_role`; `service_role` agora recebe `permission denied`; `create_conversation()`/`submit_orcamento_request()` continuam funcionando pelos caminhos legítimos (chamada função-a-função sob `SECURITY DEFINER`). | `DELIVERED + VALIDATED` | Sessão Central, P0 backend/E2E (§106/§107) | **CLOSED** |
...
## 107. Sessão Central — correção dos 2 achados do P0 — `[requires_professional_review: DELIVERED + VALIDATED]` `[hardening service_role: DELIVERED + VALIDATED]` `[mecanismo de liberação: PENDING INTEGRATION]` `[needs_you ↔ requires_professional_review: PENDING INTEGRATION]` — 15/09/2026

Aprovado pela fundadora nesta forma exata, sem reabrir discussão:
- **`requires_professional_review` (infra fail-closed)** → `DELIVERED + VALIDATED`.
- **Hardening `_create_conversation_core` (revoke `service_role`)** → `DELIVERED + VALIDATED`.
- **Mecanismo de liberação da mensagem retida** → `PENDING INTEGRATION`, aguarda decisão de produto (qual ação humana existe, definida pela Sessão Painel antes de qualquer RPC/tela nova).
- **`needs_you` ↔ `requires_professional_review`** → `PENDING INTEGRATION`, aguarda a auditoria da Sessão Painel sobre "Decisões/Precisa de você" (`doopla-intervention.ts` é compartilhado, alimenta Home).

Explicitamente registrado: o estado atual — `requires_professional_review=true` corretamente retido no backend, mas a camada de intervenção (`needs_you`) ainda não representando essa distinção — é uma inconsistência CONHECIDA e temporária, não a arquitetura final. Será reconciliada depois da auditoria da Sessão Painel, nunca contornada localmente por esta sessão.

Checkpoint desta rodada: commit `c40c445` na branch `claude/categoria-b-supabase-env-qsbdq9`, working tree limpa, migrations 0083/0084 aplicadas e validadas (10/10 testes, ver §107 abaixo). Nenhuma ação adicional tomada sobre as duas pendências — Sessão Central parada aqui, aguardando a conclusão da auditoria da Sessão Painel antes de qualquer integração entre as duas sessões.
