...
export function ProAvatarUploader({
  ...
  if (objectUrl) {
    ...
    return (
      <div className="flex flex-col gap-4">
        <div
          className="relative mx-auto overflow-hidden rounded-full bg-white/[0.03]"
        ...
        <div className="flex justify-center gap-3">
          <button type="button" onClick={handleSave} disabled={pending} className={proPrimaryButtonClass}>
        ...
        {state.error && <p className="text-center text-sm text-[var(--pro-red)]">{state.error}</p>}
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-4">
      {currentUrl ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={currentUrl} alt="" className="h-16 w-16 flex-none rounded-full object-cover" />
      ) : (
        <span className="font-pro-sub flex h-16 w-16 flex-none items-center justify-center rounded-full bg-white/10 text-xl font-bold text-[var(--pro-off)]">
          {initialsFromName(fallbackName)}
        </span>
      )}
      <div>
        <button type="button" onClick={() => fileInputRef.current?.click()} className={proGhostButtonClass}>
          Alterar foto
        </button>
