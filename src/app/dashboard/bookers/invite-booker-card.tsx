...
import { inviteBookerAction } from '../actions';
...
export function InviteBookerCard({ invites }: { invites: Invite[] }) {
  const [state, formAction, pending] = useActionState(inviteBookerAction, {});

  return (
    <section id="convites" className={cardClass}>
      <p className={eyebrowClass}>Traga sua dupla para a doopla</p>
      <p className="mt-1 text-sm text-[var(--ink)]/55">
        Já tem um booker ou alguém que te representa fora da plataforma? Convide — quando a
        conta dele for criada e o convite confirmado, vocês já ficam conectados na doopla.
      </p>

      <form action={formAction} className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
        <label className="flex flex-1 flex-col gap-1.5">
          <span className={eyebrowClass}>Nome do booker</span>
          <input
            type="text"
            name="name"
            required
