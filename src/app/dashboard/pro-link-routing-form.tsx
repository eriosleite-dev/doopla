...
// Pro re-skin (Bloco 7, P1) de LinkRoutingForm — mesma action/lógica,
// só o tema --pro-*. O original continua servindo o Booker via
// orcamento-link-card.tsx (booker-home-view.tsx), intocado.
export function ProLinkRoutingForm({
  bookers,
  currentMode,
  currentBookerId,
  onSaved,
    ...
    <form action={formAction} className="flex flex-col gap-3">
      <div className="flex flex-col gap-2.5">
        {MODE_OPTIONS.map((option) => (
          <label
            key={option.value}
            className="flex items-start gap-3 rounded-[14px] border border-[var(--pro-line)] p-3.5"
          >
            <input
              type="radio"
              name="mode"
            ...
            <span className="flex flex-col gap-0.5">
              <span className="text-[13px] font-medium text-[var(--pro-off)]">{option.label}</span>
              <span className="text-[12.5px] text-[var(--pro-tx-50)]">{option.hint}</span>
            </span>
          </label>
        ))}
      </div>
      ...
      {mode !== 'eu' && (
        <label className="flex flex-col gap-1.5">
          <span className={proLabelClass}>Booker</span>
          <select name="bookerId" defaultValue={currentBookerId ?? ''} className={proInputClass}>
